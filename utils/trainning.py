...
class dataset(Dataset):
    def __init__(self, dataset_path):
        self.images = datasets.DatasetFolder(
            root=f"./{dataset_path}/", loader=torch.load, extensions="pt"
        )
        self.dnas = datasets.DatasetFolder(
            root=f"./{dataset_path}/", loader=dna_ToTensor, extensions="txt"
        )
...
class Net(LightningModule):
    ...
    def training_epoch_end(self, outputs):
        ...
        self.logger.log_metrics(
            {"learning rate": self.scheduler._last_lr[0]}, self.current_epoch
        )
...
class experiment:
    def __init__(self, model, opts):
        ...
        self.stop = EarlyStopping(
            monitor="val_loss", patience=6, strict=False, mode="min", min_delta=6
        )
        ...
        self.trainer = Trainer(
            ...
            logger=TensorBoardLogger(
                "./logs", name="Regressor", default_hp_metric=False, log_graph=True
            ),
        )
